...
function runtime({ position = 100, rate = 4, speed = 1, stamp = 1000, finished = false, raceId = 1 } = {}) {
  return {
    race: { id: raceId },
    player: { racePlayerId: 11, positionAtEpochMs: stamp },
    playerStatus: "RACING",
...
function runPolledFrames(motion, durationMs, { position, rate, speed, stamp }, fps = 60) {
  const samples = [];
  const frameMs = 1000 / fps;
  let nextPollMs = POLL_MS;
  for (let index = 0; index < Math.round(durationMs * fps / 1000); index += 1) {
    samples.push(motion.advance(frameMs));
    if (Math.round((index + 1) * frameMs) >= nextPollMs) {
      motion.updateRuntimeState(runtime({ position: position + rate * nextPollMs / 1000, rate, speed, stamp: stamp + nextPollMs }));
      nextPollMs += POLL_MS;
    }
  }
  return samples;
...
describe("student race visual motion", () => {
  ...
  it.each([10, 15, 20])("smoothly reconciles a +%s position bonus with bounded visual velocity", (bonus) => {
    ...
    const answer = { position: 100 + bonus, rate: 5.6, speed: 1.4, stamp: 2000 };
    motion.updateRuntimeState(runtime(answer));

    const samples = runPolledFrames(motion, 16000, answer);
  ...
  it("moderates the first easy answer's correction while retaining its higher rate after feedback ends", () => {
    ...
    const later = { position: 110 + 2.8 * effectMs / 1000, rate: 2.8, speed: 0.7, stamp: 2000 + effectMs };
    const samples = [...duringFeedback, ...runPolledFrames(motion, 20000 - effectMs, later)];
    let previousPosition = 100;
    let peakRate = 0;
  ...
  it("gradually accelerates the world itself to a sustained authoritative movement rate", () => {
    const motion = createStudentRaceMotion();
    motion.updateRuntimeState(runtime());
    const faster = { position: 100, rate: 8, speed: 2, stamp: 2000 };
    motion.updateRuntimeState(runtime(faster));

    const samples = runPolledFrames(motion, 16000, faster);
  ...
  it("accepts equal-timestamp position updates without restarting from older truth", () => {
    ...
    expect(runPolledFrames(motion, 16000, { position: 110, rate: 4, speed: 1, stamp: 1000 }).at(-1).position)
      .toBeCloseTo(174, 2);
  });
  ...
  it("does not replay an unchanged snapshot when presentation props update after a long prediction", () => {
    ...
    const state = runtime();
    motion.updateRuntimeState(state);
    runPolledFrames(motion, 40000, { position: 100, rate: 4, speed: 1, stamp: 1000 });
    const latest = runtime({ position: 260, stamp: 41000 });
    motion.updateRuntimeState(latest);
    const settled = motion.advance(0).position;
    motion.updateRuntimeState({ ...latest });

    expect(motion.advance(0).position).toBe(settled);
    expect(settled).toBeCloseTo(260, 1);
  });

  it("bounds a resumed long frame and seeds a snapshot beyond the old visible window", () => {
    const motion = createStudentRaceMotion();
    motion.updateRuntimeState(runtime({ rate: 8 }));

    const resumed = motion.advance(10000).position;
    expect(resumed).toBeGreaterThan(100.7);
    expect(resumed).toBeLessThan(101.5);
  ...
  it("smoothly follows an authoritative slowdown and delayed position without reversing", () => {
    ...
    runFrames(motion, 1000);
    const slower = { position: 106, rate: 6.4, speed: 1.6, stamp: 2000 };
    motion.updateRuntimeState(runtime(slower));
    let previousPosition = 108;
    const samples = runPolledFrames(motion, 16000, slower);
  ...
  it("resets directly on a dev loop wrap or a different race", () => {
    ...
    motion.advance(1200);
    motion.updateRuntimeState({ ...runtime({ position: 990, stamp: 4000, raceId: 2 }), player: { racePlayerId: 22, positionAtEpochMs: 4000 } });
  ...
  it("settles reduced-motion runout on proof without moving authoritative position", () => {
    const motion = createStudentRaceMotion();
    const truth = runtime({ position: 999, rate: 0 });
    truth.visual.reducedMotion = true;
    motion.updateRuntimeState(truth);
    motion.updateFinishPresentation(presentation(false));
    expect(motion.advance(2000).position).toBeLessThan(1000);
    motion.updateFinishPresentation(presentation(true));
    expect(motion.advance(16).position).toBe(1006);
    expect(truth.visual.targetPosition).toBe(999);
  });
});
